...
struct s5k4h8yx_device *s5kh8yx_dev;
...
{
	int ret;
	if (NULL != s5kh8yx_dev->otp_raw_data) {
		ret = dw9761_otp_save(s5kh8yx_dev->otp_raw_data,
		DW9761_OTP_RAW_SIZE,
		DW9761_SAVE_RAW_OTP);
	...
	if (NULL != s5kh8yx_dev->otp_data) {
		ret = dw9761_otp_save(s5kh8yx_dev->otp_data,
